...
public class InitialFinalSuperStateEContentAdapter extends EContentAdapter implements CincoEContentAdapter {
  @Override
  public void notifyChanged(final Notification notification) {
    super.notifyChanged(notification);
    final Object o = notification.getNotifier();
    final Object feature = notification.getFeature();
    if ((o instanceof InternalInitialFinalSuperState)) {
      if ((Objects.equal(((InternalInitialFinalSuperState)o).eContainer(), null) && (!(o instanceof InternalGraphModel)))) {
        return;
      }
      ...
      if (feature instanceof EStructuralFeature) {
        ...
        if (_isRelevant) {
          _matched=true;
          ModelElement _element = ((InternalInitialFinalSuperState)o).getElement();
          String _name = ((EStructuralFeature)feature).getName();
          Object _oldValue = notification.getOldValue();
          final PostAttributeChangePayload<InitialFinalSuperState> eventPayload = new PostAttributeChangePayload<InitialFinalSuperState>(((InitialFinalSuperState) _element), _name, _oldValue);
          final PayloadContext<PostAttributeChangePayload<InitialFinalSuperState>, Void> eventContext = new PayloadContext<PostAttributeChangePayload<InitialFinalSuperState>, Void>("event.post.attributeChange.info_scce_cinco_product_scchart_mglid_scchart_InitialFinalSuperState", eventPayload);
          EventHub.getInstance().notifyFirst(eventContext);
          ModelElement _element_1 = ((InternalInitialFinalSuperState)o).getElement();
