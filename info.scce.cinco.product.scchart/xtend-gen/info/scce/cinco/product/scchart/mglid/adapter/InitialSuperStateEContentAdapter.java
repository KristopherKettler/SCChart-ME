...
public class InitialSuperStateEContentAdapter extends EContentAdapter implements CincoEContentAdapter {
  @Override
  public void notifyChanged(final Notification notification) {
    super.notifyChanged(notification);
    final Object o = notification.getNotifier();
    final Object feature = notification.getFeature();
    if ((o instanceof InternalInitialSuperState)) {
      if ((Objects.equal(((InternalInitialSuperState)o).eContainer(), null) && (!(o instanceof InternalGraphModel)))) {
        return;
      }
      ...
      if (feature instanceof EStructuralFeature) {
        ...
        if (_isRelevant) {
          _matched=true;
          ModelElement _element = ((InternalInitialSuperState)o).getElement();
          String _name = ((EStructuralFeature)feature).getName();
          Object _oldValue = notification.getOldValue();
          final PostAttributeChangePayload<InitialSuperState> eventPayload = new PostAttributeChangePayload<InitialSuperState>(((InitialSuperState) _element), _name, _oldValue);
          final PayloadContext<PostAttributeChangePayload<InitialSuperState>, Void> eventContext = new PayloadContext<PostAttributeChangePayload<InitialSuperState>, Void>("event.post.attributeChange.info_scce_cinco_product_scchart_mglid_scchart_InitialSuperState", eventPayload);
          EventHub.getInstance().notifyFirst(eventContext);
          ModelElement _element_1 = ((InternalInitialSuperState)o).getElement();
